...
3. **Atomic spend-and-sign**
   Mint verifies every input, marks them spent, and signs all outputs in one atomic operation. If commit fails after signing (a durability failure), the mint halts rather than return signatures — refusing to open a double-spend window is the honest failure mode.
